...
public class CommonSqlLogManager implements Interceptor{
...
//			log.debug("CommonSqlLogManager:::sql = " + sql);
////			System.out.println("CommonSqlLogManager:::param = " + sql);
//			
//			//쿼리 실행시 맵핑되는 파라메터들
//			String param = "";
...
//			}
//			log.debug("CommonSqlLogManager:::param = " + param);
////			System.out.println("CommonSqlLogManager:::param = " + param);
//			
//			//DB에다 로그 insert
			...
			if(param == null){				//파라미터가 아무것도 없을 경우
				...
				if(param instanceof Integer || param instanceof Long || param instanceof Float || param instanceof Double){
				...
				} else if(param instanceof String){	//해당 파라미터 클래스가 String일 경우(이 경우는 앞뒤에 '(홑따옴표)를 붙여야해서 별도 처리
					sql = sql.replaceFirst("\\?", "'" + param + "'");
				} else if(param instanceof Map){	//해당 파라미터가 Map일 경우
					/**
					...
					for(ParameterMapping mapping : paramMapping){
						String propValue = mapping.getProperty();		//파라미터로 넘긴 Map의 key값이 들어오게 된다.
						Object value = ((Map) param).get(propValue);	//넘겨받은 key값을 이용해 실제 값을 꺼낸다.
						if(value instanceof String){					//SQL의 ?대신에 실제 값을 넣는다. 이때String일 경우는 '를 붙여야 하기때문에 별도 처리
					...
					for(ParameterMapping mapping : paramMapping){
						String propValue = mapping.getProperty();	//해당 파라미터로 넘겨받은 사용자 정의 클래스
						Field field = paramClass.getDeclaredField(propValue);	//관련 멤버변수 Field객체 얻어옴
						...
						if(String.class == javaType){				//SQL의 ? 대신에 실제 값을 넣는다. 이때 String일 경우는 '를 붙여야 하기 때문에 별도 처리
			...
			log.debug("================================================================");
			log.debug("CommonSqlLogManager processTime : {}", processTime);
			log.debug("================================================================");
		}
	}
