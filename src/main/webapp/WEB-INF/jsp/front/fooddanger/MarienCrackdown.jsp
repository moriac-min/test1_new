    ...
    <div id="header">
                    <div class="board-head">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="active">수산물 원산지표시위반</li>
                        </ol>
                        <h1>수산물 원산지표시위반</h1>
                        ...
                        <span class="alert-info">수산물 원산지표시위반 정보는 해양수산부 산하 <a href="http://www.nfqs.go.kr" target="_blank" title="[새창]국립수산물품질관리원" class="alert-link"><strong>국립수산물품질관리원</strong></a>에서 정보를 제공받아 공표하고 있습니다.</span></p>
                    </div>
                </div>
	 ...
	 ****************************************/
	 function searchList(){
	        ...
	        $.ajax({ 
	            ...
	            dataType:'json', 
	            type:'POST',
	            url:"${contextPath}/portal/fooddanger/searchMarienCrackdownList.do", 
	 ...
	 function fn_drawList(list){
        ...
        for(var i=0; i<listLangth; i++){
            ...
            content += "<ul>";
            content += "  <a class='openinfo fancybox.iframe' data-fancybox-group='iframe' onclick=\"javascript:fn_moveDetail('"+list[i].entrps_cd+"','"+list[i].instt_cd+"','"+list[i].dayrec_no+"');\">";
            ...
            content += "      <dl>";
            content += "          <dt>위반내용</dt>";
            content += "          <dd>"+list[i].vilt_matr_dvs_nm+"</dd>";
            content += "          <dt>처분명</dt>";
            content += "          <dd>"+list[i].dsps_dtls_dvs_nm+"</dd>";
            content += "      </dl>";                            
            content += "    </li>";
            content += "    <li class='issue-date'>";
            content += "       <dl>";
            content += "          <dt>처분일</dt>";
            content += "          <dd>"+list[i].dspth_dtm+"</dd>";
            content += "      </dl>";                            
            content += "    </li>";
            content += "  </a>";
            content += "</ul>";
	 ...
	 //상세페이지팝업
    function fn_moveDetail(entrps_cd, instt_cd, dayrec_no){
    	
    	var action  = '${contextPath}/popup/marienCrackdownDetail.do?entrps_cd='+entrps_cd+"&instt_cd="+instt_cd+"&dayrec_no="+dayrec_no;
