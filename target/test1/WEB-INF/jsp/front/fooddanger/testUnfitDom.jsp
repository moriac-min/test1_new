...
<script type="text/javascript">
	...
	****************************************/
	 ...
	 ****************************************/
	 ...
	 function fn_drawList(list){
        ...
        for(var i=0; i<listLangth; i++){
            ...
            content += list[i].rtrvl_prvns+"</li>";//코드 - 분류명 처리 필요
            content += "<a class='inconsistence fancybox.iframe' data-fancybox-group='iframe' onclick=\"javascript:fn_moveDetail('"+list[i].report_no+"');\">";
            if(list[i].atch_file_no != ''){
            	content += "<li class='img-222x175' style='background-image:url("+list[i].atch_file_no+");'></li>";
            ...
            content += "</a><li class='date'>"+list[i].cret_dtm+"</li>";
            content += "</ul>";
        }
    ...
    function fn_moveDetail(report_no){
    	...
    	var action  = '${contextPath}/popup/testUnfitDetail.do?search_keyword='+report_no;
