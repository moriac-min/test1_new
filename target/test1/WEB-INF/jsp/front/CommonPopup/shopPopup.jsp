...
<script type="text/javascript">
    ...
    ****************************************/
    ...
    //목록 그리기
     function fn_drawList(list){
        ...
        for(var i=0; i<listLangth; i++){
            ...
            content += "<tr>";
            content += "    <td scope='row'>"+list[i].no+"</td>";
            content += "    <td>"+list[i].induty_cd_nm+"</td>"; 
        	content += "    <td><a href='#' onclick=\"javascript:setSelectedVal('"+list[i].lcns_no+"','"+list[i].bssh_nm+"','"+list[i].telno+"','"+list[i].addr+"','"+list[i].addr_dtl+"','"+list[i].zipno+"','"+list[i].bsn_lcns_ledg+"'); return false;\"><strong>"+list[i].bssh_nm+"</strong></a></td>";
        	content += "    <td>"+list[i].prsdnt_nm+"</td>"; 
        	content += "    <td>"+list[i].lcns_no+"</td>";
        	content += "    <td class='tl'>"+list[i].addr+" "+list[i].addr_dtl+"</td>";
        	content += "</tr>";
        }
    ...
    //허가번호, 업소명, 연락처, 주소, 상세주소, 우편번호
    function setSelectedVal(lcns_no, bssh_nm, telno, addr, addr_dtl, zipno, bsn_lcns_ledg){
    	parent.fn_callbackShop(lcns_no, bssh_nm, telno, addr, addr_dtl, zipno, bsn_lcns_ledg);
    }
</script>
